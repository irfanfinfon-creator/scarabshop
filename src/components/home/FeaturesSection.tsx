...
export function FeaturesSection() {
  return (
    <section className="py-24 bg-background-alt overflow-hidden relative">
      ...
      <div className="absolute top-0 right-0 -translate-y-1/2 translate-x-1/2 w-[500px] h-[500px] bg-accent/5 rounded-full blur-3xl pointer-events-none" />
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <div className="text-center mb-16">
          <h2 className="text-4xl lg:text-5xl font-heading font-bold text-primary mb-6 animate-fadeIn">
            Elevate Your Shopping Experience
          </h2>
          <div className="w-20 h-1.5 bg-accent mx-auto rounded-full mb-8" />
          <p className="text-xl text-secondary max-w-2xl mx-auto font-body leading-relaxed">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 auto-rows-[200px]">
            ...
            >
              <div className="relative z-10 flex flex-col h-full justify-between">
                <div>
                  ...
                  <h3 className="text-2xl font-heading font-bold text-primary group-hover:text-accent transition-colors duration-300">
                    {f.title}
                  </h3>
                  <p className="text-secondary py-2 leading-relaxed font-body text-base">
                    {f.description}
                  </p>
                </div>
              </div>
              ...
              <div className="absolute top-0 right-0 -mr-16 -mt-16 w-32 h-32 bg-accent/5 rounded-full blur-2xl group-hover:bg-accent/10 transition-colors duration-500" />
            </div>
          ))}
          ...
          {stats.map((s, i) => (
            ...
            >
              <div className="text-5xl font-body font-black text-accent mb-3 tracking-tighter group-hover:scale-110 transition-transform duration-500">
                {s.num}
              </div>
              <div className="text-primary font-body uppercase tracking-[0.2em] text-sm font-bold">
                {s.label}
              </div>

              {/* Decorative accent line */}
              <div className="absolute bottom-0 left-0 w-full h-1.5 bg-accent transform scale-x-0 group-hover:scale-x-100 transition-transform duration-500 origin-left" />
            </div>
          ))}
        </div>
      </div>
